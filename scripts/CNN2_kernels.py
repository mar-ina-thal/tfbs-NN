...
class Conv_v1(nn.Module):
    def __init__(self, kernel_size1, kernel_size2):
        ...
        self.conv1 = nn.Conv1d(in_channels=4, out_channels=5, kernel_size=kernel_size1)
        self.conv2 = nn.Conv1d(in_channels = 5, out_channels = 5, kernel_size=kernel_size2)
...
model = Conv_v1(kernel1[tf], kernel2[tf])
print(f'Initializing model with kernels: {kernel1[tf]}, {kernel2[tf]}')
...
sub_directory = f"CNN_results_balanced_kernels_200/{tf}/CNN2"
...
plt.savefig(f"{save_directory}/LOSS_AUC{tf}_CNN2_ADAM_BCEWithLogits_seq=200b.png")
plt.title(f"Loss Curves {tf}_CNN1_ADAM_BCEWithLogits_seq=200b.png")


helper_functions.plot_auroc(results)
plt.savefig(f"{save_directory}/AUROC{tf}_CNN2_ADAM_BCEWithLogits_seq=200b.png")
plt.title(f"AUROC {tf}_CNN1_ADAM_BCEWithLogits_seq=200b.png")

helper_functions.plot_f_score(results)
plt.savefig(f"{save_directory}/F1_SCORE{tf}_CNN2_ADAM_BCEWithLogits_seq=200b.png");
